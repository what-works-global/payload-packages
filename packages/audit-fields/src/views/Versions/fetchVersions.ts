// Vendored from payload@3.84.1 `packages/next/src/views/Version/fetchVersions.ts`
// (MIT). The default versions view's data helpers are not exported from
// `@payloadcms/next`, so they are reproduced here unchanged (minus `fetchVersion`,
// which the list view does not use).
import {
  logError,
  type PaginatedDocs,
  type PayloadRequest,
...
export const fetchLatestVersion = async <TVersionData extends object = object>({
  ...
  const entityConfig = collectionSlug
    ...
    : globalSlug
      ? req.payload.config.globals.find((global) => global.slug === globalSlug)
      : undefined
