 ...
 * same store. `envCache` picks the file cache in development, which is what
 * bridges the separate module graphs `next dev` runs the proxy and the server
 * in, and the Vercel Runtime Cache in production.
 *
 ...
 */
export const cache: RedirectsCache = envCache({
