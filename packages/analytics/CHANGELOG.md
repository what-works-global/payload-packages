...
- 23b5eed: Add `usePostHog()` and `onPostHogReady()` to the `@whatworks/analytics/posthog` entry for reacting to PostHog readiness without polling.
...
- d759047: Fix `capture()` (and consent toggling) silently no-opping in the Next.js App Router.
