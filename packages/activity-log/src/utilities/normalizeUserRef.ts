import type { ActivityUserRef } from '../types.js'

/**
 * Accepts whatever a log entry holds for the `user` field and normalizes it to a
 * `{ relationTo, value }` ref. The plugin stores the polymorphic shape, but be
 * lenient about plain IDs (single `relationTo` via override) and populated docs
 * (custom fetch depth).
 */
export const normalizeUserRef = (
  raw: unknown,
  fallbackCollection?: string,
): ActivityUserRef | null => {
  if (raw == null) {
    return null
  }
