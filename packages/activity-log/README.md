...
- **Trash.** On collections with `trash: true`, moving to the trash logs `trash` (linked to the document under the trash route), restoring logs `restore`, and emptying the trash logs `delete` with a snapshot.
