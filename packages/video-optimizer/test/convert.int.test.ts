...
import { getRenditionUrl, getVideoSources } from '../src/exports/frontend.js'
import { resolutionPresets, videoOptimizerPlugin, widthPresets } from '../src/index.js'
...
const versionRows = (doc: JsonObject): VersionRow[] =>
  Array.isArray(doc.renditions) ? (doc.renditions as VersionRow[]) : []
...
beforeAll(async () => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'payload-video-optimizer-test-'))
  ...
  const config = await buildConfig({
    ...
    plugins: [
      ...
      videoOptimizerPlugin({
        ...
        jobs: { taskSlug: 'video-convert-guarded' },
        onConversionComplete: (outcome) => {
          outcomes.push(outcome)
        },
        presets: { webm: {} },
      ...
      videoOptimizerPlugin({
        ...
        jobs: { taskSlug: 'video-convert-failing' },
        presets: { webm: {} },
      }),
    ],
    secret: 'test-secret',
...
describe.skipIf(!ffmpegAvailable)('async conversion (requires ffmpeg)', () => {
  it('responds with a queued record, then the job stores a WebM sidecar next to the intact original', async () => {
    ...
    expect((created as JsonObject).videoConversion).toMatchObject({ status: 'queued' })
    ...
    expect(doc.videoConversion).toMatchObject({
      originalFilename: 'clip.mp4',
      originalFilesize: sampleMp4.byteLength,
      originalMimeType: 'video/mp4',
      status: 'complete',
    })
    expect((doc.videoConversion as JsonObject).encodeDurationMs).toBeGreaterThan(0)
    ...
    expect(sidecar.isVideoDerivative).toBe(true)
    expect(sidecar.renditionPreset).toBe('webm')
  ...
  it('converts QuickTime sources and VP8 targets', async () => {
    ...
    expect(movDoc.videoConversion).toMatchObject({ status: 'complete' })
  ...
  it('generates one rendition per preset and serves them via the frontend helpers', async () => {
    ...
    expect(getRenditionUrl(populated)).toBe(sources[0].src)
    expect(getRenditionUrl(populated, '240p')).toBe(sources[1].src)
  ...
  it('regenerates one preset via the endpoint, replacing only that rendition', async () => {
    ...
    const endpoint = payload.config.endpoints.find(
      (e) => e.path === '/video-convert/regenerate' && e.method === 'post',
    )
    ...
    expect(after.videoConversion).toMatchObject({ status: 'complete' })
    ...
    const sidecars = await payload.find({
      collection: 'ladder',
      depth: 0,
      where: { isVideoDerivative: { equals: true }, renditionPreset: { equals: '144p' } },
    })
  ...
  it('converts even when the metadata group is disabled', async () => {
    ...
    expect(doc.videoConversion).toBeUndefined()
  ...
  it('keeps renditions when an ordinary update carries a stale renditions snapshot', async () => {
    ...
    await payload.update({ id: created.id, collection: 'media', data: { renditions: [] } })
    await expect(fetchDoc('media', sidecarId!)).resolves.toBeTruthy()
  })
  ...
  it('re-encodes a preset whose rendition was deleted behind the plugin', async () => {
    ...
    const job = (await payload.jobs.queue({
      input: {
        collection: 'media',
        docId: created.id,
        generation: Number(doc.renditionGeneration),
    ...
    expect(repaired.videoConversion).toMatchObject({ status: 'complete' })
  ...
  it('refuses to queue another conversion while one is still in flight', async () => {
    ...
    expect((await fetchDoc('deferred', created.id as number)).videoConversion).toMatchObject({
      status: 'queued',
    })

    const endpoint = payload.config.endpoints.find(
      (e) => e.path === '/video-convert-deferred/regenerate' && e.method === 'post',
    )
    const user = await payload.create({
      collection: 'users',
  ...
  it('regenerates the sidecar when the file is replaced, and clears it for non-videos', async () => {
    ...
    const sidecars = await payload.find({
      collection: 'media',
      depth: 0,
      where: { filename: { contains: 'v2' }, isVideoDerivative: { equals: true } },
    })
    ...
    expect((cleared.videoConversion as JsonObject).status).toBeNull()
  ...
  it('preserves conversion metadata when a document is re-saved without a new file', async () => {
    ...
    expect((resaved as JsonObject).videoConversion).toMatchObject({
  ...
  it('bypasses WebM uploads untouched, reporting already-webm', async () => {
    ...
    expect((doc as JsonObject).videoConversion).toMatchObject({ status: null })
  ...
  it('records input-too-large at upload time without queueing a job', async () => {
    ...
    expect((doc as JsonObject).videoConversion).toMatchObject({
  ...
  it('lets the shouldConvert predicate veto conversions, reported but not stamped', async () => {
    ...
    expect((vetoed as JsonObject).videoConversion).toMatchObject({
      skippedReason: null,
      status: null,
    })
...
describe.skipIf(process.platform === 'win32')('deterministic paths (no ffmpeg needed)', () => {
  it('records output-larger and keeps the intact original, without a sidecar', async () => {
    ...
    expect(doc.videoConversion).toMatchObject({ skippedReason: 'output-larger', status: 'skipped' })
  ...
  it('records a failed job on the document and keeps the original stored', async () => {
    ...
    expect(doc.videoConversion).toMatchObject({ status: 'failed' })
    expect(String((doc.videoConversion as JsonObject).error)).toMatch(/ffmpeg/)
    expect(versionRows(doc)).toHaveLength(0)
  })
})
