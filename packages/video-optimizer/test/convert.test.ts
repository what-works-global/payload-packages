...
const countTempDirs = (): number =>
  fs.readdirSync(fixtureDir).filter((name) => name.startsWith('payload-video-optimizer-')).length

beforeAll(() => {
  fixtureDir = fs.mkdtempSync(path.join(os.tmpdir(), 'video-optimizer-fakebin-'))
