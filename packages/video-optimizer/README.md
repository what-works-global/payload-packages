...
Tests (`pnpm test`) generate their video fixtures with ffmpeg and run the full suite — `ffmpeg-static` is a devDependency, so no system install is needed. Set `FFMPEG_PATH` to run them against a distribution build instead. If the encode suite ever reports _skipped_, the binary didn't download: pnpm 10 blocks install scripts, so run `pnpm approve-builds` and pick `ffmpeg-static`.
