 ...
 *
 * All of them need the `renditions` relationships populated with their `url`s,
...
const warnUnpopulated = (): void => {
  ...
  console.warn(
    `[payload-video-optimizer] this document has renditions rows, but none are populated, so the original file will be served instead. Query one level deeper: depth 1 reading the upload collection directly, depth 2 when the video is reached through another document (page → media → rendition).`,
  )
}

const populatedRenditions = (doc: null | undefined | VideoDocLike): VideoVariant[] => {
  const rows = doc?.renditions
...
const warnIfAspectUnmatched = (
  ...
  // eslint-disable-next-line no-console
  console.warn(
    `[payload-video-optimizer] aspect ${JSON.stringify(aspect)} asks for a shape this document has no renditions in, so it changes nothing. Encode one with the plugin's \`portrait\` option (or widthPresets with an aspectRatio).`,
  )
}
...
const warnIfAscending = (sizes: VideoSizeRule[]): void => {
  ...
  // eslint-disable-next-line no-console
  console.warn(
    `[payload-video-optimizer] getVideoSourceSet was given ascending minWidth rules. The browser takes the first source whose media query matches, so list the widest breakpoint first or every viewport will get the smallest rendition.`,
  )
}
 ...
 *
 * ```tsx
 * <video controls src={getRenditionUrl(media, '720p') ?? media.url ?? undefined} />
 * ```
 */
