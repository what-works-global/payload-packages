...
import { renditionRows } from '../hooks/sidecar.js'
...
export const createRegenerateEndpoint = (
  ...
  handler: async (req) => {
    ...
    const keptRows = doc && preset ? renditionRows(doc).filter((row) => row.preset !== preset) : []
    const generation = Number(doc?.[RENDITION_GENERATION_FIELD_NAME] ?? 0) + 1
    const data: JsonObject = doc
      ? {
          ...(entry.config.metadataFields
            ...
            : {}),
          [RENDITION_GENERATION_FIELD_NAME]: generation,
          [RENDITIONS_FIELD_NAME]: keptRows,
        }
      : {}
