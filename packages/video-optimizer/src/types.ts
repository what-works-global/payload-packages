...
export interface DispatchJob {
  ...
  docId: number | string
  /**
   * Value of the document's `renditionGeneration` counter when this job was queued. The
...
export interface VideoOptimizerConfig {
  /**
   ...
   */
  collections?: Record<string, true | VideoOptimizerCollectionOverrides> | string[]
  /**
  ...
  jobs?: {
     ...
     * to `'video-convert'`; only needs changing when running two plugin
  ...
  /**
   * Inject a read-only `videoConversion` sidebar group (status, original filename/mime/
   ...
   * from the source via `renditions` rows (`{ preset, video }`). Declaration
...
/** {@link VideoOptimizerConfig} with every default applied and validated. */
export interface ResolvedVideoOptimizerConfig {
  encoding: Pick<VideoEncodingOptions, 'aspectRatio' | 'maxHeight' | 'maxWidth'> &
    Required<Omit<VideoEncodingOptions, 'aspectRatio' | 'maxHeight' | 'maxWidth'>>
...
/** Lifecycle of one upload's conversion, stored in the `videoConversion` metadata group. */
export type ConversionStatus = 'complete' | 'failed' | 'queued' | 'skipped'

/** Shape of the `videoConversion` metadata group on target documents. */
