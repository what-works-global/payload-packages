...
/**
 * `payload-video-sizes <url> --selector <css>` — measures a slot in a real browser and
...
const USAGE = `
payload-video-sizes <url> --selector <css-selector> [options]
...
Example:
  payload-video-sizes http://localhost:3000/blog --selector "[data-slot=card]"
`.trim()
...
const loadPlaywright = async (): Promise<PlaywrightLike> => {
  try {
    ...
    console.error(
      `[payload-video-optimizer] this command needs Playwright, which the plugin deliberately does not depend on.\n  pnpm add -D playwright && pnpm exec playwright install chromium`,
    )
