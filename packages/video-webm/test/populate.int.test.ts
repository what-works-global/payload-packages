 ...
 * `webmVersions` returns a video with no renditions attached and every helper
...
import { EXCLUDE_WEBM_DERIVATIVES, videoWebmPlugin } from '../src/index.js'
...
const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'video-webm-populate-'))
...
beforeAll(async () => {
  const config = await buildConfig({
    ...
    plugins: [videoWebmPlugin({ collections: ['media'], dispatch: 'inline' })],
    secret: 'test-secret',
  ...
  // The stamp hook resets webmVersions on any write carrying a file, so link the
  // rendition with a second, file-less update — the same shape the job writes.
  await payload.update({
    id: source.id,
    collection: 'media',
    data: {
      webmVersions: [{ height: 360, preset: '640w', video: rendition.id, width: 640 }],
    } as never,
  })
...
const RECIPE = { filename: true, mimeType: true, url: true, webmVersions: true }
...
describe('the README populate recipe', () => {
  ...
  it('serves only the original when webmVersions is left out', async () => {
    // The first trap: webmVersions is not a relationship, so it is easy to drop when
    ...
    expect((doc as { webmVersions?: unknown }).webmVersions).toBeUndefined()
    expect(getVideoSourceSet(doc, { sizes: '100vw' }).map((source) => source.preset)).toEqual([null])
  })
  ...
  it('serves nothing at all when filename is left out, because url derives from it', async () => {
    ...
    const doc = await hero({ media: { mimeType: true, url: true, webmVersions: true } })
  ...
  it('applies the same field list to the renditions, since a rendition is a media doc', async () => {
    const doc = await hero({ media: RECIPE })
    const rendition = (doc as { webmVersions: { video: { url: string } }[] }).webmVersions[0]
