...
export const sweepSizes = async (
  ...
  if (samples.length === 0) {
    throw new Error(
      `[payload-video-webm] no element matched ${JSON.stringify(selector)} at any viewport between ${min} and ${max}px. Is the page loaded, and is the slot rendered at every width?`,
    )
  }
...
export class SizesDriftError extends Error {
  ...
  constructor(selector: string, sizes: string, mismatches: SizesMismatch[], suggestion: string) {
    ...
    super(
      [
        `[payload-video-webm] the sizes string for ${JSON.stringify(selector)} no longer matches the layout.`,
