 ...
 */
export const WebmConversionPanel: React.FC<{
    ...
    const load = async (): Promise<void> => {
      try {
        ...
        const doc = (await response.json()) as {
          filesize?: number
          mimeType?: string
          videoWebm?: null | Record<string, unknown>
          webmVersions?: { preset?: string; skippedReason?: string; video?: unknown }[] | null
        }
        if (cancelled) {
          return
        }

        const videoWebm = doc.videoWebm ?? {}
        const renditions: Rendition[] = []
        for (const row of doc.webmVersions ?? []) {
        ...
        const status = typeof videoWebm.status === 'string' ? videoWebm.status : null
        setState({
          encodeDurationMs:
            typeof videoWebm.encodeDurationMs === 'number' ? videoWebm.encodeDurationMs : null,
          error: typeof videoWebm.error === 'string' ? videoWebm.error : null,
          mimeType: typeof doc.mimeType === 'string' ? doc.mimeType : null,
          originalFilesize: Number(doc.filesize ?? 0),
          renditions,
          skippedReason:
            typeof videoWebm.skippedReason === 'string' ? videoWebm.skippedReason : null,
          status,
        })
